...
ImplementMode에서 모델이 tool call 대신 코드 블록만 답변한 경우에는 응답 아래에 `위 변경안을 실제 파일에 적용하시겠습니까?` 질문과 `예, 파일에 적용` / `아니오, 버리기` 버튼이 표시됩니다. `예, 파일에 적용`을 누르면 채팅 응답을 다시 안전한 패치 형식으로 변환한 뒤 실제 워크스페이스 파일에 적용합니다. 패치가 이미 적용된 응답에는 적용/버리기 버튼을 다시 표시하지 않습니다.

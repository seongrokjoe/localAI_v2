...
ImplementMode에서 모델이 tool call 대신 코드 블록만 답변한 경우에는 응답 아래에 `변경안 적용` 버튼이 표시됩니다. 이 버튼은 채팅 응답을 다시 안전한 패치 형식으로 변환한 뒤, `파일 변경 승인 대기 중` 상태를 표시하고 VS Code 승인 팝업을 거쳐 파일에 적용합니다.

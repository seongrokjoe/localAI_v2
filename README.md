...
ImplementMode 응답이 끝나면 확장은 먼저 수정 대상 파일을 확정하고, 해당 파일의 현재 원문을 워크스페이스에서 직접 다시 읽습니다. 기존 파일은 줄 번호와 시작/종료 앵커를 포함한 패치를 받아 실제 파일 범위를 검증하고, 형식 누락이나 범위 불일치가 있으면 오류를 포함해 최대 3회 자동 재시도합니다. 이 검증이 끝난 경우에만 실제 절대 경로와 변경 전후 미리보기, `예, 파일에 적용` / `아니오, 버리기` 버튼이 표시됩니다. `예, 파일에 적용`을 누른 뒤에는 LLM을 다시 호출하지 않고 이미 검증된 패치를 실제 파일에 적용합니다. 검증 이후 파일이 달라졌다면 적용을 중단하며, 실제 디스크 바이트까지 검증된 경우에만 `실제 파일 변경 완료`가 표시되고 버튼이 사라집니다.

기존 파일 변경은 `startLine/endLine`, `startAnchor/endAnchor`, `replacementText`를 사용하며 확장이 현재 파일에서 변경 범위를 직접 추출합니다. 이전 `originalText/replacementText` 형식도 호환되며, 줄바꿈과 공백 차이는 대상이 한 곳으로 확정될 때만 보정합니다. VS Code가 감지한 파일 인코딩과 줄바꿈 스타일을 유지하고, 저장 후 디스크 내용을 다시 읽어 검증하며, 내용이 반영되지 않았으면 같은 인코딩과 BOM으로 직접 저장을 재시도합니다. 새 파일 생성에만 전체 파일 내용 적용을 사용합니다. 워크스페이스 루트와 실제 절대 경로, 저장 단계는 `Company Code AI: 마지막 패치 진단 보기`에서 확인할 수 있습니다. Makefile Tools 등 다른 VS Code 확장은 필수 의존성이 아닙니다.

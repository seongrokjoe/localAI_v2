...
ImplementMode에서 모델이 tool call 대신 코드 블록만 답변한 경우에는 응답 아래에 `변경안 적용` 버튼이 표시됩니다. 이 버튼은 채팅 응답을 다시 안전한 패치 형식으로 변환한 뒤, VS Code 승인 팝업을 거쳐 파일에 적용합니다.

기존 파일 변경은 `originalText`와 `replacementText` 기반으로 적용하며, 기존 파일의 줄바꿈 스타일을 맞춰 변경 범위를 최소화합니다. 새 파일 생성에만 전체 파일 내용 적용을 사용합니다.

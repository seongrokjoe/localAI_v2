...
ImplementMode 응답에 코드 블록이 있으면 편집기 메인 영역에 변경 작업대가 자동으로 열립니다. 왼쪽 패널에는 AI 제안 블록과 체크박스가 표시되고, 오른쪽에는 원본을 복제한 로컬 작업 파일이 VS Code 네이티브 편집기로 열립니다. 이 과정에서 LLM 서버를 다시 호출하지 않습니다.
...
native tool calling을 지원하는 서버에는 구조화된 `submitChangeBlocks` 요청을 사용합니다. 지원하지 않는 서버에서는 JSON 또는 일반 Markdown 코드 블록을 AI 후보로 추출합니다. 명확히 매핑되지 않은 경로와 범위는 자동 적용하지 않으며 사용자가 직접 연결합니다. Makefile Tools 등 다른 VS Code 확장은 필수 의존성이 아닙니다.

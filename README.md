...
ImplementMode에서 모델이 tool call 대신 코드 블록만 답변한 경우에는 `아직 실제 파일은 변경되지 않았습니다` 안내와 `예, 파일에 적용` / `아니오, 버리기` 버튼이 표시됩니다. `예, 파일에 적용`을 누르면 채팅 응답을 다시 안전한 패치 형식으로 변환한 뒤 실제 워크스페이스 파일에 적용합니다. 실제 디스크 바이트까지 검증된 경우에만 `실제 파일 변경 완료`가 표시되고 버튼이 사라집니다. 패치를 만들 수 없으면 `파일은 변경되지 않았습니다`라고 표시합니다. 저장이나 검증에 실패하면 성공 처리하지 않고 패치 진단 확인과 재시도/버리기 버튼을 제공합니다.

기존 파일 변경은 `originalText`와 `replacementText` 기반으로 적용하며, VS Code가 감지한 파일 인코딩과 줄바꿈 스타일을 기준으로 변경 범위를 최소화합니다. VS Code 저장 후 디스크 내용을 다시 읽어 검증하고, 내용이 반영되지 않았으면 같은 인코딩과 BOM으로 직접 저장을 재시도합니다. 새 파일 생성에만 전체 파일 내용 적용을 사용합니다. 마지막 적용 경로와 저장 단계는 `Company Code AI: 마지막 패치 진단 보기`에서 확인할 수 있습니다.

...
VS Code에서 Git 리포지터리 또는 솔루션 루트 폴더를 엽니다. 프로젝트가 크면 `Company Code AI: 활성 스코프 설정`으로 작업할 솔루션, 프로젝트, 폴더를 좁히는 것이 좋습니다. `전체 워크스페이스 사용 (스코프 없음)`은 활성 스코프 없이 전체 폴더를 사용한다는 뜻입니다. 목록에 원하는 하위 폴더나 프로젝트 파일이 없으면 `폴더/파일 직접 선택...`을 사용합니다.
...
ImplementMode에서 모델이 tool call 대신 코드 블록만 답변한 경우에는 응답 아래에 `위 변경안을 실제 파일에 적용하시겠습니까?` 질문과 `예, 파일에 적용` / `아니오, 버리기` 버튼이 표시됩니다. `예, 파일에 적용`을 누르면 채팅 응답을 다시 안전한 패치 형식으로 변환한 뒤 실제 워크스페이스 파일에 적용하고 저장 및 검증까지 수행합니다. 대상 파일에 저장되지 않은 변경이 있으면 적용을 중단하므로 먼저 저장한 뒤 다시 적용하세요. 패치가 이미 적용된 응답에는 적용/버리기 버튼을 다시 표시하지 않습니다.

기존 파일 변경은 `originalText`와 `replacementText` 기반으로 적용하며, VS Code가 감지한 파일 인코딩과 줄바꿈 스타일을 기준으로 변경 범위를 최소화합니다. 새 파일 생성에만 전체 파일 내용 적용을 사용합니다.
